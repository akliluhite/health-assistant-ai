...
st.markdown(
    ...
    /* MAIN BACKGROUND */
    .stApp {{
        background: linear-gradient(rgba(255,255,255,0.30), rgba(255,255,255,0.30)), url("data:image/jpg;base64,{img}");
...
# =====================================================
# HEADER (Fixed: Removed empty lines inside HTML strings)
# =====================================================
st.markdown(
    """<div class="header-card"><h2 style="color:#0f766e; margin-bottom:8px; margin-top:0;">🏥 Clinical Informatics Intelligence Center</h2><p style="color:#475569; margin:0; font-size:1rem;">Machine Learning Statistical Inference Gateway</p></div>""",
    unsafe_allow_html=True
)
...
if model_ready:
    ...
    if compute_btn:
        if not selected_clean_symptoms:
            ...
            with col1:
                st.markdown(f'<div class="clinical-metric"><div class="clinical-label">Inferred Classification Prognosis</div><div class="clinical-value">{prediction}</div></div>', unsafe_allow_html=True)
            with col2:
                st.markdown(f'<div class="clinical-metric"><div class="clinical-label">Statistical Weight Confidence</div><div class="clinical-value">{confidence:.1f}%</div></div>', unsafe_allow_html=True)

            disease_profile = DISEASE_INFO.get(prediction, DISEASE_INFO.get(prediction.strip()))
            if disease_profile:
                st.markdown(f'<div class="clinical-info-bin"><strong>Pathology Profile:</strong> {disease_profile["desc"]}<br><br><strong>Target Department Referral:</strong> {disease_profile["specialist"]}<br><strong>Recommended Actions:</strong> {disease_profile["treatment"]}</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="clinical-info-bin"><strong>Pathology Profile:</strong> Custom clinical profile undefined in base database maps. Consult external health references.</div>', unsafe_allow_html=True)
    else:
